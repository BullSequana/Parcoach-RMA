...
int main(int argc, char* argv[])
{
  ...
  if(my_rank == 0)
  {
    // Get in my_value from target 1
    MPIX_Get_notify(&window_buffer[0], 1, MPI_INT, 1, 0, 1, MPI_INT, window, 1);
    MPIX_Win_wait_notify(window, 2);
  }

  if(my_rank == 1)
  {
    // Get in my_value from target 0
    MPIX_Win_wait_notify(window, 1);
    MPIX_Get_notify(&window_buffer[0], 1, MPI_INT, 0, 20, 1, MPI_INT, window, 2);
  }
