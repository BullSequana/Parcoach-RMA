...
void insert_interval_tree(Interval_tree *root, Interval *i)
{
  ...
  /*If root's low value is smaller than new interval's low value -> go to left subtree*/
  if(low_bound < low_bound_root)
  {
    ...
    root->tree_max_depth = max(root->tree_max_depth, root->left->tree_max_depth + 1);
  }
 /*Else go to right subtree */
    ...
    /* Update max depth if needed */
    root->tree_max_depth = max(root->tree_max_depth, root->right->tree_max_depth + 1);
  }
