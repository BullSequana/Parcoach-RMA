...
int fc_MPI_Comm_size(MPI_Comm comm, int *size)
{
  ...
  else // language_program == FORT
  {
     ...
     * to convert it */
    MPI_Comm c_comm = MPI_Comm_f2c(*((int *)comm));
...
int fc_MPI_Send(const void *buf, int count, MPI_Datatype datatype,
...
int fc_MPI_Irecv(void *buf, int count, MPI_Datatype datatype,
...
int fc_MPI_Type_contiguous(int count, MPI_Datatype oldtype,
                           MPI_Datatype *newtype)
{
  /* Since the datatypes are created by the C library, no action needed */
  return MPI_Type_contiguous(count, oldtype, newtype);
}

int fc_MPI_Type_commit(MPI_Datatype *datatype)
{
  /* Since the datatype is created by the C library, no action needed */
  return MPI_Type_commit(datatype);
}

int fc_MPI_Type_free(MPI_Datatype *datatype)
{
  /* Since the datatype is created by the C library, no action needed */
...
rma_analyzer_state *rma_analyzer_get_state(MPI_Win win)
{
  ...
  if (NULL == state)
  {
    LOG(stderr,"Error : no state found in %s, exiting now\n", __func__);
    exit(EXIT_FAILURE);
  }
         ...
         && (get_up_bound(itv) < (state->win_base + state -> win_size))))
  {
...
#endif
    {
      ...
      if (overlap_itv != NULL)
      {
          ...
          fprintf(stderr, "Error when inserting memory access of type %s ", access_type_to_str(access_type));
          fprintf(stderr, "from file %s at line %d ", filename, fileline);
          fprintf(stderr, "with already inserted interval of type %s ", access_type_to_str(overlap_access_type));
          fprintf(stderr, "from file %s at line %d.\n", overlap_filename, overlap_fileline);
    ...
    {
      LOG(stderr,"Fine, there is no intersection between the intervals in this List !\n");
    }
...
void rma_analyzer_check_communication(Interval *received_itv, MPI_Win win)
{
  ...
  fc_MPI_Irecv(received_itv, 1, state->interval_datatype, MPI_ANY_SOURCE,
               state->mpi_tag + state->count_epoch,
               state->win_comm, &mpi_request);
...
void rma_analyzer_init_comm_check_thread(MPI_Win win)
{
  ...
  (void) pthread_create(&state->threadId, NULL,
                        &rma_analyzer_comm_check_thread, win);
}
...
{
  int id = (tag_id - RMA_ANALYZER_BASE_MPI_TAG) /
           RMA_ANALYZER_MAX_EPOCH_NUMBER;
  win_mpi_tag_id[id] = 0;
}
...
void rma_analyzer_start(void *base,MPI_Aint size, MPI_Comm comm, MPI_Win *win, Lang language)
{
  ...
  fc_MPI_Type_contiguous(struct_size, MPI_BYTE,
                         &(new_state->interval_datatype));
  fc_MPI_Type_commit(&new_state->interval_datatype);
  ...
  if (tag_id < 0)
  {
    LOG(stderr,"Error : no tag to use. Please check if you have allocated more than %d windows simultaneously.\n",
        RMA_ANALYZER_MAX_WIN);
  }
  new_state->mpi_tag = RMA_ANALYZER_BASE_MPI_TAG +
                       (RMA_ANALYZER_MAX_EPOCH_NUMBER * tag_id);
